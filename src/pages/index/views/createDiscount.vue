<template>
  <div class="createDiscountPage">
    <!-- 折扣切换tab -->
    <van-tabs sticky v-model="formData.type" @change="step=0" class="tabs__content">
      <van-tab title="整单折扣">
        <van-cell-group>
          <van-field
            label="折扣额度(1-100)% ："
    ...
    <!-- 底部操作按钮 -->
    <van-tabbar>
      <van-button :disabled="submitStatus" type="primary" size="large" @click="toRule">生成折扣券</van-button>
    </van-tabbar>
    <van-popup v-model="notesStatus" position="right" :overlay="false">
      <van-steps :active="parseInt(step)-1" active-color="#51b8cb">
      ...
      <div class="step2" v-if="step===1">
        <div class="notesContain">
          <!-- 内容列表 -->
          <div v-if="ruleList.length" class="tags">
            <span
        ...
        <div class="nextStep-contain">
          <span class="nextStep backBtn" @click="step=0">取消</span>
          <span class="nextStep" @click="toNotes">添加备注</span>
        </div>
      </div>
      <!-- 备注对话框 -->
